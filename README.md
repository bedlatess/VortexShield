...
早期真实样本不足时，可以先写入一批标记清晰的真人行为引导样本。它会生成“真人正确对齐”和“真人拖拽但未对齐”两类数据，用于让模型先学到自然拖拽的速度、停顿和微颤特征：

```powershell
.\.venv\Scripts\python.exe scripts\seed_training_logs.py --passed 50 --failed 30
```
...
- JSONL 日志读取与坏行容错
- `bootstrap_human_passed` / `bootstrap_human_slider_miss` 样本识别
