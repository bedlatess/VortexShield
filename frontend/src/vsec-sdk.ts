...
const EVENT_MOVE: TrackEventType = 0;
const EVENT_DOWN: TrackEventType = 1;
const EVENT_UP: TrackEventType = 2;
...
class CaptchaSDK {
  ...
  private readonly onComplete?: (bundle: CaptchaSubmitBundle) => void;
  private readonly onSuccess?: (signature: string, response: unknown) => void;
  private readonly onFailure?: (reason: string, response: unknown) => void;
  private readonly onError?: (error: unknown) => void;
  private readonly locale: LocaleCode;

  private simulateBot: boolean;
  private root: HTMLDivElement;
  private bodyEl: HTMLDivElement;
  private footerEl: HTMLDivElement;
  private state: SDKState = "idle";
  private challenge: CaptchaChallengeData | null = null;
  private checkboxChallenge: CheckboxChallengeData | null = null;
  private startTime = performance.now();
  private tracks: TrackPoint[] = [];
  private cleanupCallbacks: Array<() => void> = [];
  private lastMoveSampleAt = 0;
  private activePointerArea: HTMLElement | null = null;
  private refreshTimer: number | null = null;

  private sliderStage: HTMLDivElement | null = null;
  private sliderPiece: HTMLImageElement | null = null;
  private sliderTrack: HTMLDivElement | null = null;
  private sliderFill: HTMLDivElement | null = null;
  private sliderKnob: HTMLButtonElement | null = null;
  private sliderX = 0;
  private sliderDragStartClientX = 0;
  private sliderDragStartX = 0;
  ...
  private renderSilentLoading(text: string): void {
    this.bodyEl.innerHTML = `
  ...
  private renderSilentSuccess(): void {
    this.bodyEl.innerHTML = `
  ...
  private async activateCheckboxChallenge(challenge: CheckboxChallengeData): Promise<void> {
    ...
    const moveHandler = (event: MouseEvent) => this.recordSampledEvent(event, EVENT_MOVE, box);
  ...
  private async activateSliderChallenge(challenge: CaptchaChallengeData): Promise<void> {
    this.resetInteractionState();
    ...
    this.sliderStage = shell.querySelector<HTMLDivElement>(".vsec-stage");
    this.sliderPiece = shell.querySelector<HTMLImageElement>(".vsec-piece");
    this.sliderTrack = shell.querySelector<HTMLDivElement>(".vsec-track");
    this.sliderFill = shell.querySelector<HTMLDivElement>(".vsec-track-fill");
    this.sliderKnob = shell.querySelector<HTMLButtonElement>(".vsec-knob");
  ...
  private bindSliderEvents(): void {
    ...
    const downHandler = (event: MouseEvent | TouchEvent) => {
      event.preventDefault();
    ...
    const upHandler = async (event: MouseEvent | TouchEvent) => {
  ...
  private async submitCheckboxVerify(): Promise<void> {
  ...
  private async submitVerify(bundle: CaptchaSubmitBundle): Promise<void> {
    try {
  ...
  private async failAndRefresh(reason: string, response: unknown): Promise<void> {
  ...
  private async fetchPrecheckKey(): Promise<PrecheckKeyResponse["data"]> {
    ...
    const body = (await response.json()) as PrecheckKeyResponse;
  ...
  private applySliderX(nextX: number, dragging: boolean): void {
  ...
  private clampSliderX(value: number): number {
  ...
  private clampPieceY(value: number, challenge: CaptchaChallengeData): number {
  ...
  private recordSampledEvent(event: MouseEvent, eventType: TrackEventType, area: HTMLElement): TrackPoint | null {
  ...
  private recordPointerEvent(event: MouseEvent, eventType: TrackEventType, area: HTMLElement): TrackPoint {
    ...
    const track: TrackPoint = [point.x, point.y, timestamp, eventType];
    this.tracks.push(track);
    return track;
  }

  private recordSampledSyntheticTrack(clientX: number, clientY: number, eventType: TrackEventType): TrackPoint | null {
    const now = performance.now();
    if (now - this.lastMoveSampleAt < 4) {
      return null;
    }
  ...
  private recordSyntheticTrack(clientX: number, clientY: number, eventType: TrackEventType): TrackPoint {
  ...
  private toLocalPoint(clientX: number, clientY: number, area: HTMLElement): { x: number; y: number } {
  ...
  private normalizeCheckboxChallenge(challenge: ChallengeData | null): CheckboxChallengeData {
  ...
  private t(key: keyof typeof I18N.zh): string {
    return I18N[this.locale][key];
  }

  private localizedPrompt(prompt: string | undefined, fallbackKey: keyof typeof I18N.zh): string {
    if (this.locale === "en") {
  ...
  private async encryptHybridPayload(
    payload: VerifyPlainPayload | PrecheckPlainPayload,
    rsaPublicKeyPem: string,
  ): Promise<EncryptedHybridPayload> {
  ...
  private async importRsaPublicKey(pem: string): Promise<CryptoKey> {
    ...
    const keyBuffer = der.buffer.slice(der.byteOffset, der.byteOffset + der.byteLength) as ArrayBuffer;
  ...
  private async collectFingerprint(): Promise<CaptchaFingerprint> {
    ...
    const webdriverDescriptorTampered =
      Boolean(descriptor) && (typeof descriptor?.get !== "function" || descriptor?.set !== undefined);
...
function isSliderChallenge(value: unknown): value is CaptchaChallengeData {
  if (!value || typeof value !== "object") {
    return false;
  }
  const challenge = value as Partial<CaptchaChallengeData>;
  return (
    challenge.captcha_type === "SLIDER" &&
...
function isCheckboxChallenge(value: unknown): value is CheckboxChallengeData {
  if (!value || typeof value !== "object") {
    return false;
  }
  const challenge = value as Partial<CheckboxChallengeData>;
  return (
    challenge.captcha_type === "CLICK_CHECKBOX" &&
    typeof challenge.captcha_token === "string" &&
    typeof challenge.rsa_public_key === "string"
  );
}
